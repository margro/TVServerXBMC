...
namespace TVServerXBMC
{
  ...
  public class ShareInfo
  {
    ...
    public ShareInfo(string server, string netName, string path, ShareType shareType, string remark)
    {
    ...
    public DirectoryInfo Root
    {
      get
      {
        if (IsFileSystem)
        {
          if (null == _servername || 0 == _servername.Length)
  ...
  public class ShareCollection : ReadOnlyCollectionBase
  {
    ...
    public static string GetUncPathForLocalPath(string directory)
    {
      ...
      if (!IsValidLocalFilePath(directory) || !Directory.Exists(directory))
        return string.Empty;
      ...
      nRet = WNetGetUniversalName( directory, UNIVERSAL_NAME_INFO_LEVEL, ref rni, ref bufferSize);
      ...
      if (ERROR_MORE_DATA == nRet)
      {
        ...
        try
        {
          nRet = WNetGetUniversalName( directory, UNIVERSAL_NAME_INFO_LEVEL, pBuffer, ref bufferSize);
          ...
          if (null != shi)
          {
            ...
            if (null != share)
            {
              ...
              if (null != path && 0 != path.Length)
              {
                ...
                directory = Path.Combine(share.ToString(), directory);
              }
            }
          }
    ...
    /// <returns></returns>
    public static ShareInfo PathToShare(string fileName)
    {
      if (null == fileName || 0 == fileName.Length) return null;

      fileName = Path.GetFullPath(fileName);
      if (!IsValidLocalFilePath(fileName)) return null;
    ...
    protected void Add(ShareInfo share)
    {
      InnerList.Add(share);
    }
    ...
    {
      InnerList.Add(new ShareInfo(_server, netName, path, shareType, remark));
    }
    ...
    {
      get { return (ShareInfo)InnerList[index]; }
    }

    /// <summary>
    /// Returns the <see cref="ShareInfo"/> which matches a given local path
    /// </summary>
    ...
    public ShareInfo this[string path]
    {
      get
      {
        ...
        if (!IsValidLocalFilePath(path)) return null;
    ...
    public void CopyTo(ShareInfo[] array, int index)
    {
